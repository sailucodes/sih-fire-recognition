...
async function syncBackendFirms(days = 1, isManual = false) {
    // If a sync is already running, cancel it so requests don't collide
    ...
    try {
        ...
        const label = days === 1 ? "Live Satellite Pass (Today)" : `Past ${days} Days`;
        showToast(`Loaded ${label}: ${allEvents.length} real thermal sources classified by AI!`, "success");
        return allEvents.length;
    } catch (err) {
        if (err.name === "AbortError") {
            console.log("[Live Sync] Request aborted for newer selection.");
            return 0;
        }
        ...
        if (allEvents.length === 0) {
            ...
            if (alertsList) {
                alertsList.innerHTML = `<div style="text-align: center; color: var(--danger-color); padding: 20px; font-size: 13px;"><i class="fa-solid fa-triangle-exclamation"></i> Telemetry sync failed (${escapeHTML(err.message)}). Ensure backend server is running on port 8000.</div>`;
            }
        }
        throw err;
    } finally {
